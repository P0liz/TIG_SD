...
class MnistClassifier(nn.Module):
    ...
    def __init__(self, img_size=32):
        ...
        self.conv_layers = nn.Sequential(
            ...
            nn.Sequential(nn.Conv2d(64, 64, 3, padding=1), nn.ReLU()),
            nn.MaxPool2d(2),
        )
        self.dense_layers = nn.Sequential(
            nn.Sequential(nn.Dropout(), nn.Linear(3136, 512), nn.ReLU()),
            nn.Sequential(
                nn.Dropout(),
...
class CifarClassifier(nn.Module):
    ...
    def __init__(self, img_size=32):
        ...
        self.conv_layers = nn.Sequential(
            ...
            nn.Sequential(nn.Conv2d(128, 128, 3, padding=1), nn.ReLU()),
            nn.MaxPool2d(2),
        )
        self.dense_layers = nn.Sequential(
            nn.Sequential(nn.Dropout(), nn.Linear(2048, 1024), nn.ReLU()),
            nn.Sequential(
                nn.Dropout(),
...
class MobileNet(nn.Module):
    def __init__(self):
        ...
        self.layerM_01 = ConvBN_mobilenet(
            3, 32, 2
        )  # if MNIST import fails, probably just because of channels
